...
const categorize = (message: string): ErrorReport['category'] => {
	...
	if (message.includes('fetch') || message.includes('network') || message.includes('Failed to load')) return 'network';
	if (message.includes('TypeError') || message.includes('ReferenceError') || message.includes('SyntaxError')) return 'javascript';
	return 'unknown';
};
...
const isDeprecatedReactWarningMessage = (message: string): boolean => {
	const deprecatedPatterns = [/componentWillReceiveProps/, /componentWillMount/, /componentWillUpdate/, /UNSAFE_componentWill/];
...
const hasRelevantSourceInStack = (stack?: string): boolean => {
	...
	const hasSourceFiles = lines.some((line) => SOURCE_FILE_PATTERNS.some((pat) => pat.test(line)));
	if (hasSourceFiles) return true;

	const isAllVendor = lines.every((line) => line.trim() === '' || line.includes('Error') || VENDOR_PATTERNS.some((pat) => pat.test(line)));
	return !isAllVendor;
};
...
class GlobalErrorDeduplication {
	private reportedErrors = new Map<string, { timestamp: number; precedence: ErrorPrecedence; reported: boolean }>();
	...
	private isHigherPrecedence(newPrec: ErrorPrecedence, existingPrec: ErrorPrecedence): boolean {
	...
	shouldReport(context: ErrorContext, immediate = false): { shouldReport: boolean; reason?: string } {
		this.maybeCleanup();
...
class ErrorReporter {
	...
	private createConsoleInterceptor(method: ConsoleMethod, original: ConsoleNative, prefix: string) {
		return (...args: ConsoleArgs) => {
			...
			try {
				...
				const level = method === 'warn' && message.includes(REACT_WARNING_PATTERN) ? 'warning' : 'error';
	...
	private setupConsoleInterceptors() {
		...
		console.error = this.createConsoleInterceptor('error', this.originalConsoleError!, CONSOLE_ERROR_PREFIX);
		console.warn = this.createConsoleInterceptor('warn', this.originalConsoleWarn!, WARNING_PREFIX);
	}
	...
	private createErrorPayload(data: Partial<ErrorReport> & { message: string }): ErrorReport {
	...
	private filterError(context: ErrorContext): ErrorFilterResult {
		...
		if (level === 'error' && message.includes('Uncaught Error') && !hasRelevantSourceInStack(stack)) {
		...
		if (level === 'error' && source && VENDOR_PATTERNS.some((pattern) => pattern.test(source)) && !hasRelevantSourceInStack(stack)) {
		...
		if (!deduplicationResult.shouldReport) return { shouldReport: false, reason: deduplicationResult.reason };
	...
	private parseStackTrace(stack?: string): string {
		...
		try {
			...
			for (const line of lines) {
				...
				const componentMatch = line.match(/at (\w+) \(.*?\/src\/(.*?):(\d+):(\d+)\)/);
				if (componentMatch) {
...
const formatConsoleArgs = (args: unknown[]): string => {
	return args
		.map((arg) => (typeof arg === 'string' ? arg : typeof arg === 'object' && arg ? JSON.stringify(arg, null, 2) : String(arg)))
		.join(' ');
};

type ImmediatePayload = Pick<ErrorReport, 'message' | 'stack' | 'url' | 'timestamp' | 'level' | 'category'>;

const createImmediateErrorPayload = (message: string, level: 'warning' | 'error'): ImmediatePayload => ({
	message,
...
const shouldReportImmediate = (context: ErrorContext): boolean => {
	...
	// Skip deprecated React lifecycle warnings
	const deprecatedPatterns = [/componentWillReceiveProps/, /componentWillMount/, /componentWillUpdate/, /UNSAFE_componentWill/];
	if (level === 'warning' && deprecatedPatterns.some((pattern) => pattern.test(message))) return false;
...
if (typeof window !== 'undefined') {
	...
	const createImmediateInterceptor = (original: ConsoleNative, prefix: string, defaultLevel: 'warning' | 'error') =>
	...
	console.warn = createImmediateInterceptor(originalWarn, '[WARNING]', 'warning') as WrappedConsoleFn;
	(console.warn as WrappedConsoleFn).__errorReporterWrapped = true;
	console.error = createImmediateInterceptor(originalError, '[CONSOLE ERROR]', 'error') as WrappedConsoleFn;
